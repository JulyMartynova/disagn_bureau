...
export const fetchProjects = async (): Promise<Project[]> => {
    try {
        const response = await axios.get<{ projects: Project[] }>("http://localhost:8081/support"); // Указываем тип ответа
...
export const fetchProjectDetails = async (projectId: number): Promise<ProjectDetails> => {
    try {
        const response = await axios.get<ProjectDetails>(`http://localhost:8081/support/${projectId}`); 
